...
%During the training, we also kept track of the average value of the Energy over
%all the sequences and the value of the partition function at every epoch.
%Then we combine the two to obtain the log-likelihood:
%\begin{equation} 
%\logL(\vec{\theta}) = -\expval{E(\vec{\theta})}_{\mathrm{data}} -
%\log(Z(\vec{\theta}))
%\end{equation}
%to be sure that this quantity is increased during the training and then
%that it reaches a stable value.
